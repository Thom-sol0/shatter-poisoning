...
class VNodeSharingPoison(VNodeSharing):
    ...
    def defender_forward_averaging(self, data):
        ...
        if self.param_values is None:
            ...
            # Track received real nodes to avoid duplicates
            self.received_real_nodes = set()
            ...
            # Add self to received real nodes
            self.received_real_nodes.add(self.uid)
        ...
        if real_node_id is not None and real_node_id in self.received_real_nodes:
            # Skip this data to avoid duplicates from the same real node
            return
        ...
        # Mark this real node as received
        if real_node_id is not None:
            self.received_real_nodes.add(real_node_id)
    ...
    def defender_finish_forward_averaging(self, peer_deques):
        ...
        self.received_real_nodes = None  # Reset received real nodes tracking
