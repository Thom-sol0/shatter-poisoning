...
class VNodeSharingPoison(VNodeSharing):
    ...
    def defender_forward_averaging(self, data):
        """
        Computes and collects parameter values for geometric mean-based aggregation.
    ...
    def get_geomean_model(self):
        """
        Computes the element-wise geometric mean of collected weight values
        and returns a model state dictionary with these geometric means.
        """
        # Create a tensor to hold the geometric mean values
        geomean_weights = torch.zeros(self.total_length, dtype=torch.float32, device=self.device)
    
        # Calculate the geometric mean for each weight position
        corrupted_indices = []
        for idx in range(self.total_length):
            if idx in self.param_values and len(self.param_values[idx]) > 0:
                # Filter out any remaining NaN/Inf values before computing geometric mean
                ...
                if len(filtered_values) > 0:
                    ...
                    # Use the sign-preserving geometric mean: sign(mean) * geomean(|values|)
                    ...
                    # Compute geometric mean using log-space for numerical stability
                    log_mean = np.mean(np.log(abs_values))
                    geomean_abs = np.exp(log_mean)
                    
                    # Determine the overall sign (majority vote)
        ...
        # Final validation of geometric mean weights
    ...
    def defender_finish_forward_averaging(self, peer_deques):
        ...
        Applies geometric mean-based defense against poisoning attacks.
        """
        ...
        # Calculate the weight-wise geometric mean and update model
        geomean_model = self.get_geomean_model()
        self.model.load_state_dict(geomean_model)
