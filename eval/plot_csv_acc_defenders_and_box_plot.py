...
def get_list_of_max_test_acc(results):
    """Get a list of maximum test accuracy for each dataframe"""
    assert 'test_acc' in results[0].columns
    maxes = []
    for df in results:
        max_acc = df['test_acc'].max()
...
def create_max_accuracy_box_plot(results_path, folder_data):
    """
    Create a box plot of maximum test accuracies across different experiments
    ...
    folder_data : dict
        Dictionary mapping folder names to lists of max test accuracies
    """
    ...
    for folder in sorted_folders:
        if folder_data[folder]:  # Only include folders with data
            labels.append(folder)
            data.append(folder_data[folder])
    ...
    # Add styling
    plt.title('Maximum Test Accuracy Distribution by Experiment')
    ...
    # Save the plot
    plt.savefig(os.path.join(results_path, "max_accuracy_distribution.pdf"), dpi=600)
    print(f"Box plot saved to {os.path.join(results_path, 'max_accuracy_distribution.pdf')}")
...
def plot_results(results_path, config_path=None):
    ...
    # Dictionary to collect max accuracies for box plot
    all_max_accuracies = {}
    ...
    for folder in folders:
        ...
        #function test for get_list_of_max_test_acc
        max_test_accs = get_list_of_max_test_acc(results)
        print(f"Max test accuracies for folder {folder}: {max_test_accs}")

        # Get max test accuracies for this folder
        if results:
            if adversarial_nodes and node_ids:
                # Filter out adversarial nodes for the box plot data
                defender_results = [res for res, node_id in zip(results, node_ids) if node_id not in adversarial_nodes]
                all_max_accuracies[folder] = get_list_of_max_test_acc(defender_results)
    ...
    # After all folder processing is complete, create the box plot
    if all_max_accuracies:
        create_max_accuracy_box_plot(results_path, all_max_accuracies)
